...
export default function Navbar({ page }: { page?: page }) {
    ...
    return (
        <nav className="flex items-center justify-between">
            ...
            <button onClick={() => setOpen(true)} className="md:hidden text-neutral-700 cursor-pointer">
                <IconMenu2 className="w-6 h-6" />
            </button>
            <div className={cn("fixed top-0 left-0 w-full h-full bg-gray-900/50 p-4", open ? '' : 'hidden')}>
                <div className="bg-white rounded p-4">
                    <div className="flex items-center justify-between">
                        <Link href="/" className="font-bold text-xl text-gray-900">jstn<span className="text-primary-500">lt.</span></Link>
                        <button onClick={() => setOpen(false)} className="cursor-pointer">
                            <IconX className="w-6 h-6 text-neutral-700" />
                        </button>
                    </div>
